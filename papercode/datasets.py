...
class CamelsTXT(Dataset):
    ...
    use_runoff ： bool
        If True, add 'runoff' into inputs
    """
    ...
    def _load_data(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.use_runoff:  # ADD
            start_date = self.dates[0] - pd.DateOffset(days=self.seq_length)  # ADD
        else:
            start_date = self.dates[0] - pd.DateOffset(days=self.seq_length - 1)
        end_date = self.dates[1]
        df = df[start_date:end_date]
        df = df.dropna()  # ADD
        # df = df.drop((df[df['QObs(mm/d)'] < 0]).index)  # ADD
        # print("origin:", df.shape)  # TEST

        # use all meteorological variables as inputs
        if self.use_runoff:  # ADD
            # df_runoff = df.loc[:end_date - pd.DateOffset(days=1), 'QObs(mm/d)']
            df_runoff = df.loc[:df.index[-1] - pd.DateOffset(days=1), 'QObs(mm/d)']
            runoff_next_day = df_runoff.index[0] + pd.DateOffset(days=1)  # NOTE:其他要靠后一天
            df_next_day = df[runoff_next_day:]
            if df_runoff.shape != df_next_day['vp(Pa)'].shape:
                print(self.basin)
                print(df_runoff)
                print(df_next_day['vp(Pa)'])
            x = np.array([
                df_runoff.values,
                df_next_day['prcp(mm/day)'].values, df_next_day['srad(W/m2)'].values, df_next_day['tmax(C)'].values,
                df_next_day['tmin(C)'].values, df_next_day['vp(Pa)'].values
            ]).T
            # print("runoff_x:", x.shape, x)  # TEST
            y = np.array([df.loc[runoff_next_day:, 'QObs(mm/d)'].values]).T
            # store first and last date of the selected period (including warm_start)
            self.period_start = df_runoff.index[0] + pd.DateOffset(days=14)
            self.period_end = df.loc[runoff_next_day:, 'QObs(mm/d)'].index[-1]
            # if self.basin == "02096846":
            #     print(df_runoff)
            #     print("runoff:", x.shape, x)  # TEST
            #     print("runoff:", y.shape, y)  # TEST
            # print("runoff_y:", y.shape, y)  # TEST
        else:
            x = np.array([
                df['prcp(mm/day)'].values, df['srad(W/m2)'].values, df['tmax(C)'].values,
                df['tmin(C)'].values, df['vp(Pa)'].values
            ]).T
            # print("no runoff:", x.shape, x)  # TEST
            y = np.array([df['QObs(mm/d)'].values]).T
            # print("no runoff:", y.shape, y)  # TEST

            # store first and last date of the selected period (including warm_start)
            self.period_start = df.index[0] + pd.DateOffset(days=self.seq_length - 1)
