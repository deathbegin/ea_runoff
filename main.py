...
from papercode.datautils import (add_camels_attributes, load_attributes,
                                 rescale_features, cal_scaler)
from papercode.ealstm import EALSTM
from papercode.lstm import LSTM
from papercode.metrics import calc_nse, calc_mse, calc_kge
...
def get_args() -> Dict:
    ...
    parser.add_argument('mode',
                        choices=["train", "evaluate_global", "evaluate_single", "evaluate_single2", "eval_robustness",
                                 "test"])  # CHANGE
    ...
    # ADD
    parser.add_argument('--basin_list', type=str, help="Input pub basin_list address")
    parser.add_argument('--val_best_epoch', type=int, default=30, help="best model for val dataset")
    parser.add_argument('--use_runoff', type=bool, default=False, help="wether use runoff as input")
...
def _setup_run(cfg: Dict) -> Dict:
    ...
    run_name = f'run_{month}{day}_{hour}{minute}_ealstm{not cfg["concat_static"]}_seq{cfg["seq_length"]}_basin{cfg["basin_list"]}_seed{cfg["seed"]}'
    cfg['run_dir'] = Path(__file__).absolute().parent / "runs" / "kfold" / run_name
    if cfg["use_runoff"]:
        cfg['run_dir'] = Path(__file__).absolute().parent / "runs_runoff" / "kfold" / run_name
...
def _prepare_data(cfg: Dict, basins: List) -> Dict:
    ...
    cfg["train_file"] = cfg["train_dir"] / f'{cfg["basin_list"]}train_data.h5'  # CHANGE
...
def train(cfg):
    ...
    basins = get_basin_list(cfg["basin_list"])
    cal_scaler(cfg["camels_root"], basins, use_runoff=cfg["use_runoff"])  # ADD:must!!!!
...
# def evaluate_global(user_cfg: Dict):
#     """Train model for a single epoch.
#
#     Parameters
#     ----------
...
#
#     basins = get_basin_list(user_cfg["basin_list"])
#
...
#
#     # load trained model
#     nse_all = pd.DataFrame(columns=['epoch', 'nse_median', 'nse_mean'])
#     for epoch in range(1, run_cfg["epochs"] + 1):  # ADD
...
#
#         # prepare data for training
#         user_cfg["db_path"] = str(user_cfg["run_dir"] / "attributes.db")
#
#         # create .h5 files for train and validation data
#         user_cfg["val_file"] = run_cfg["val_dir"] / f'{user_cfg["basin_list"]}train_data.h5'  # CHANGE
#         create_h5_files(camels_root=user_cfg["camels_root"],
#                         out_file=user_cfg["val_file"],
#                         basins=basins,
...
#
#         # prepare PyTorch DataLoader
#         ds = CamelsH5(h5_file=user_cfg["val_file"],
#                       basins=basins,
#                       db_path=user_cfg["db_path"],
#                       concat_static=run_cfg["concat_static"],
#                       cache=user_cfg["cache_data"],
...
#         nse_epoch.append(nse)
#         # for basin in tqdm(basins):
#         #     ds_eval = CamelsTXT(camels_root=user_cfg["camels_root"],
#         #                         basin=basin,
...
#         #                         concat_static=run_cfg["concat_static"],
#         #                         db_path=db_path)
#         #     loader = DataLoader(ds_eval, batch_size=1024, shuffle=False, num_workers=4)
#         #
#         #     preds, obs = evaluate_basin(model, loader)
#         #     # ADD
#         #     nse = calc_nse(obs[obs >= 0], preds[obs >= 0])
...
#
#         # ADD
#         nse_median = np.median(nse_epoch)
#         nse_mean = np.mean(nse_epoch)
#         print(epoch, nse_median, nse_mean)
#         nse_all = nse_all.append(pd.DataFrame({'epoch': [epoch], 'nse_median': [nse_median], 'nse_mean': [nse_mean]}))
#         # ADD
#     # ADD
#     if not user_cfg["run_dir"].is_dir():
#         user_cfg["val_dir"] = user_cfg["run_dir"] / "val"
#         user_cfg["val_dir"].mkdir(parents=True)
#     nse_all.to_csv(user_cfg["run_dir"] / "val" / f"nse_all.txt", encoding="utf-8", sep=',',
#                    index=False)  # 第一列是中值，第二列是均值，第三列全部值
#     # ADD


def evaluate_single(user_cfg: Dict):
    """Train model for a single epoch.
    ...
    basins = get_basin_list(user_cfg["basin_list"])
    known_basins = get_basin_list(run_cfg["basin_list"])  # ADD:must!!!!
    cal_scaler(run_cfg["camels_root"], known_basins, use_runoff=run_cfg["use_runoff"])  # ADD:must!!!!
    ...
    # load trained model
    nse_all = pd.DataFrame(columns=['epoch', 'nse_median', 'nse_mean'])
    nse_median_best = {'epoch': -1, 'val': 0}
    nse_mean_best = {'epoch': -1, 'val': 0}
    for epoch in range(1, run_cfg["epochs"] + 1):  # ADD
        weight_file = user_cfg["run_dir"] / f'model_epoch{epoch}.pt'
        model.load_state_dict(torch.load(weight_file, map_location=DEVICE))
        ...
        for basin in tqdm(basins):
            ds_eval = CamelsTXT(camels_root=user_cfg["camels_root"],
                                ...
                                concat_static=run_cfg["concat_static"],
                                db_path=db_path,
                                use_runoff=run_cfg["use_runoff"])
            loader = DataLoader(ds_eval, batch_size=1024, shuffle=False, num_workers=12)  # CHANGE:num_workers=4
        ...
        print(epoch, nse_median, nse_mean)
        nse_all = nse_all.append(pd.DataFrame({'epoch': [epoch], 'nse_median': [nse_median], 'nse_mean': [nse_mean]}))
        if nse_median > nse_median_best['val']:
            nse_median_best['epoch'] = epoch
            nse_median_best['val'] = nse_median
        if nse_mean > nse_mean_best['val']:
            nse_mean_best['epoch'] = epoch
            nse_mean_best['val'] = nse_mean
        # ADD
    print("nse median best:", nse_median_best, "nse mean best:", nse_mean_best)
    # ADD
    user_cfg["val_dir"] = user_cfg["run_dir"] / f"val_{user_cfg['basin_list']}_no0"
    if not user_cfg["val_dir"].is_dir():
        user_cfg["val_dir"].mkdir(parents=True)
    nse_all.to_csv(user_cfg["val_dir"] / f"nse_all.txt", encoding="utf-8", sep=',',
                   index=False)  # 第一列是中值，第二列是均值，第三列全部值
    # ADD
...
def test(user_cfg: Dict):
    ...
    # load trained model
    weight_file = user_cfg["run_dir"] / f'model_epoch{user_cfg["val_best_epoch"]}.pt'
    model.load_state_dict(torch.load(weight_file, map_location=DEVICE))

    user_cfg["pub_test_dir"] = user_cfg["run_dir"] / f"pub_test{user_cfg['basin_list']}_no0"
    if not os.path.isdir(user_cfg["pub_test_dir"]):
        user_cfg["pub_test_dir"].mkdir(parents=True)
    ...
    for basin in tqdm(basins):
        ...
        preds, obs = test_basin(model, loader)
        preds = preds[1:]
        obs = obs[1:]
        # print(preds,obs)

        # print(basin, preds.shape, obs.shape)

        nse = calc_nse(obs, preds)  # TEST:似乎不影响？ NOTE:val时去掉了负值，在测试时显然不公平
        rmse = calc_mse(obs, preds)
        kge = calc_kge(obs, preds)
        # print(nse,rmse,kge)
        ...
        metric_results = metric_results.append(
            pd.DataFrame({'basin': [basin], 'nse': [nse], 'rmse': [rmse], 'kge': [kge]}))

        date_range = pd.date_range(start=ds_test.period_start + pd.DateOffset(days=1), end=ds_test.period_end)
        # print(date_range)
        df = pd.DataFrame(data={'qobs': obs.flatten(), 'qsim': preds.flatten()}, index=date_range)
        basin_dir = user_cfg["pub_test_dir"] / "detail" / basin
        if not os.path.isdir(basin_dir):
            basin_dir.mkdir(parents=True)
        df.to_csv(basin_dir / f"test_results_{user_cfg['val_best_epoch']}.txt",
                  encoding="utf-8", sep=',', index_label="start_date")
        # results[basin] = df

    metric_results.to_csv(user_cfg["pub_test_dir"] / f"test_all_{user_cfg['val_best_epoch']}.txt",
                          encoding="utf-8",
                          sep=',', index=False)
    ...
    with open(user_cfg["pub_test_dir"] / f"test_statistics_{user_cfg['val_best_epoch']}.txt", 'w+') as f:
    ...
    # if not os.path.isdir(user_cfg["pub_test_dir"]):
    #     user_cfg["pub_test_dir"].mkdir(parents=True)
    # nse_all.to_csv(user_cfg["pub_test_dir"] / f"nse_test_{user_cfg['val_best_epoch']}.txt", encoding="utf-8", sep='\t',
    #                index=False, header=False)
    # with open(user_cfg["pub_test_dir"] / f"nse_test_{user_cfg['val_best_epoch']}.txt", 'r+') as f:
    #     context = f.read()
    #     f.seek(0, 0)
    #     f.write(f"nse_median:{nse_median}\n")
    #     f.write(f"nse_mean:{nse_mean}\n")
    #     f.write(context)
    #
    # # ADD
    # # ADD
    ...
    # if not os.path.isdir(user_cfg["pub_test_dir"]):
    #     user_cfg["pub_test_dir"].mkdir(parents=True)
    # mse_all.to_csv(user_cfg["pub_test_dir"] / f"mse_test_{user_cfg['val_best_epoch']}.txt", encoding="utf-8", sep='\t',
    #                index=False, header=False)
    # # ADD
...
def test_basin(model: nn.Module, loader: DataLoader) -> Tuple[np.ndarray, np.ndarray]:
    """Evaluate model on a single basin
    ...
    with torch.no_grad():
        ...
        preds = rescale_features(preds.numpy(), variable='output')
        obs = obs.numpy()
        # set discharges < 0 to zero # TEST:似乎不影响？NOTE:val时负值用作0，在测试时可能不公平
        # preds[preds < 0] = 0
